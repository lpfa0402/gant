	...
	gantt._assign_project_dates = function (task, from, to) {
		...
		if (taskTiming.$no_start) {
			if (from && from != Infinity) {
				...
				// Fix Bug  插件再自动更新Project任务时间后，不会调用BeforeTaskDisplay方法，
				// 没有时机刷新任务显示。这里增加以下
...
var ResizerCell = (function (_super) {
	...
	function ResizerCell(parent, config, factory) {
		...
		_this._clearListeneres = function(){
			...
			this.$domEvents.detach(document,"touchmove", _this._moveHandler);
			this.$domEvents.detach(document,"touchmove", _this._startOnMove);
			this.$domEvents.detach(document,"touchend", _this._cancelDND);
		...
		_this._startDND = function(e){
			...
			//手机端适配
			_this.$domEvents.attach(document, "touchmove", _this._moveHandler);
			_this.$domEvents.attach(document, "touchend", _this._upHandler);
		};
		...
		_this._downHandler = function (e) {
			...
			//手机端适配
			_this.$domEvents.attach(document, "touchmove", _this._startOnMove);
			_this.$domEvents.attach(document, "touchend", _this._cancelDND);
